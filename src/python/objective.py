...
def set_objective(model, class_vars, config):
    ...
    for teacher in config.ALL_TEACHERS:
        for day_idx in range(len(config.DAYS)):
            daily_load = sum(v for (sec, grp, subj, tc, d, s, rm), v in class_vars.items() if tc == teacher and d == day_idx)
    ...
    for section in config.SECTIONS:
        for day_idx in range(len(config.DAYS)):
            for i in range(len(config.ALL_SLOTS) - 1):
                # --- FIX IS IN THE NEXT TWO LINES ---
                theory_at_i = sum(v for (s, grp, subj, tc, d, s_idx, rm), v in class_vars.items() if s == section and d == day_idx and s_idx == i and 'Lab' not in subj)
                theory_at_i1 = sum(v for (s, grp, subj, tc, d, s_idx, rm), v in class_vars.items() if s == section and d == day_idx and s_idx == i + 1 and 'Lab' not in subj)
    ...
    for section in config.SECTIONS:
        for day_idx in range(len(config.DAYS)):
            for slot_idx, slot in enumerate(config.ALL_SLOTS):
                ...
                sum_gA = sum(v for (sec, grp, subj, tc, d, s, rm), v in class_vars.items() if sec == section and grp == 'A' and d == day_idx and s == slot_idx)
                sum_gB = sum(v for (sec, grp, subj, tc, d, s, rm), v in class_vars.items() if sec == section and grp == 'B' and d == day_idx and s == slot_idx)
    ...
    # 4 & 5. Penalize more than one lab session per day (for group and section)
    for section in config.SECTIONS:
        for day_idx in range(len(config.DAYS)):
            # Section penalty
            ...
            for slot_idx, slot in enumerate(config.ALL_SLOTS):
                ...
                sum_labs = sum(v for (sec, grp, subj, tc, d, s, rm), v in class_vars.items() if sec == section and d == day_idx and s == slot_idx)
            ...
            for group in config.GROUPS:
                group_labs_today = sum(v for (sec, grp, subj, tc, d, s, rm), v in class_vars.items() if sec == section and grp == group and d == day_idx)
